...
DocMergeForge can rebuild a saved project's explicit `selected_files` list from the numbered PDF/DOCX sources currently present in its configured source folders. The same synchronization planner and guarded persistence path are exposed through both the desktop **Synchronize Project Sources** workflow and the `docmergeforge project-sync` CLI command.

It is designed around a review-first rule: **preview is the default; mutation requires explicit approval.**

## Desktop workflow

Start the normal desktop application:

```bash
docmergeforge-gui
```

Choose **Synchronize Project Sources**, select the saved project JSON, and review the read-only proposal. The desktop preview shows:

- current and proposed selected-file counts;
- every added path;
- every removed path;
- whether existing files are reordered;
- the complete proposed `selected_files` order;
- duplicate PDF part numbers;
- duplicate DOCX part numbers;
- missing PDF/DOCX part numbers for kinds that are present;
- whether the proposal is safe to apply; and
- whether numbered sources are complete for the available kinds.

The Apply button is disabled when same-kind duplicate part numbers make the proposal ambiguous. Missing parts remain review evidence but do not by themselves block a metadata update.

If the proposal contains removals, clicking **Apply synchronization** does not immediately write the project. A second confirmation lists the paths to be removed from `selected_files`. Declining that confirmation performs no write.

An approved desktop apply carries the exact project-file revision captured before preview into `apply_project_sync(...)`, creates the same versioned project backup as the CLI, and refuses stale on-disk changes rather than overwriting them. Synchronization does not start a merge automatically; run normal dry-run/preflight afterward.
...
A CLI apply with an ambiguous same-kind duplicate set exits with code `2` before removal approval, backup creation, or project replacement. The desktop leaves Apply disabled. Resolve/rename/remove the unintended duplicate source candidate, preview again, and only then consider applying the new selection.
...
A CLI preview reports fields including:
...
Paths can reveal private workstation information. Treat saved preview logs and screenshots as potentially sensitive metadata.

## Apply additions and reordering

After reviewing the CLI preview:

```bash
docmergeforge project-sync --project "./Book.json" --apply
```

If the proposal is unambiguous and only adds paths and/or changes the deterministic order, the command can apply it directly. The desktop uses its **Apply synchronization** button for the equivalent operation.

Before a changed apply is allowed to write, DocMergeForge verifies that the project file still has the exact content revision captured when the workflow loaded it. This is stricter than semantic object comparison: even byte-level drift such as external JSON reformatting is treated as a stale snapshot and requires a fresh preview. The existing semantic re-load comparison remains as an additional defense for callers that do not supply an exact revision token.
...
An unchanged but ambiguous proposal is still refused when an apply is requested. The absence of a metadata diff does not make duplicate source identity safe.
...
For that reason, the CLI refuses an apply that contains removals unless a second explicit approval is supplied:
...
The desktop enforces the same two-step policy with a separate confirmation dialog after the preview's Apply action. In both interfaces, review every item in `removed` before authorizing it. Removal approval removes those paths from project metadata only; it does not delete the source files from disk.

Duplicate-part ambiguity is checked before the removal-approval gate. `--allow-removals` or the desktop removal confirmation cannot override `safe_to_apply=false` and must never be used as a way to pick between duplicate source candidates.

## Write-safety behavior

The shared apply path has additional safeguards:
...
- desktop handled failures are recorded in local diagnostics and surfaced in the UI without silently retrying the write.
...
2. Open the desktop synchronization preview or run CLI `project-sync` without `--apply`.
3. Review `current`, `proposed`, `added`, `removed`, and `reordered`.
4. Confirm the proposal is safe to apply and both same-kind duplicate lists are empty.
5. Review missing parts and numbered completeness; understand that incomplete numbering can still be synchronized but cannot be treated as publication readiness.
6. Confirm that the expected part range is still correct.
7. If no removals are proposed, apply after review when desired.
8. If removals are proposed, verify each path and approve the separate desktop confirmation or CLI `--allow-removals` only when all removals are intentional.
...
13. Only then run the full publication command or corresponding desktop merge workflow.
...
5. run a synchronization preview again if useful;
6. run project dry-run/preflight and review all resolved inputs before publishing.
