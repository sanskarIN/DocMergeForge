...
Then open the host computer's LAN address from the phone/tablet browser and add the token as a one-time query parameter:

```text
http://HOST-LAN-IP:8765/?token=YOUR_LONG_RANDOM_TOKEN
```

The browser stores the token in session storage and removes it from the visible URL. Merge requests send it using the `X-DocMergeForge-Token` header.
...
These automated tests prove application-level behavior in CI; they are not a substitute for manual browser/device acceptance on every Android/iOS/browser version.
