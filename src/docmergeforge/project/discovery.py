...
def discover_project_sources(project: MergeProject) -> list[InputDocument]:
    ...
    A strictly nested output directory is excluded before scanner hashing/inspection so
    previously published artifacts, reports, and transaction residue cannot be fed back
    into source discovery or incur unnecessary document inspection.
    """
    ...
    exclude_roots = [output] if output_is_strictly_nested else []
    return scan(project.source_folders, recursive=True, exclude_roots=exclude_roots)
