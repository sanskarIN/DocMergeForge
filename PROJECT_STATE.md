...
- Checkpoint immediately before this state-file commit: `0a54268f59b223833eed449562f7975376d94768`
...
For the documentation checkpoint inspected before the latest `what_changed.md` update:

- the recursive `main` tree contained the new documentation, checker, tests, and Quality workflow update;
- GitHub combined commit status exposed no passing status set;
...
The new checker/test/CI source is implementation evidence only until an actual run is observed.
...
3. Continue the guarded-project follow-up: consider safely routing `MergeApplicationService.discover()` through `discover_project_sources()` to remove remaining duplicated nested-output logic after regression evidence is available.
4. Consider a project-file revision/concurrency guard if multi-writer project editing is intended; project persistence is currently documented as last-writer-wins rather than collaborative locking.
5. Evaluate whether the desktop project/order UI should expose the same preview/apply/second-removal-approval synchronization model.
6. Continue independent release-gate work for native-office fidelity, measured multi-gigabyte stress, human accessibility, clean-machine packaged applications, signing, and notarization.
