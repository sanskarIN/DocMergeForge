...
from docmergeforge.docx import libreoffice_uno_acceptance
from docmergeforge.docx.fidelity import FidelityCapability
from docmergeforge.docx.libreoffice_uno_merge import LibreOfficeUnoMergeResult
...
def test_supervised_uno_acceptance_accepts_matching_body_evidence(
    tmp_path: Path,
    ...
    evidence = libreoffice_uno_acceptance.run_libreoffice_uno_acceptance(
        [first, second],
        output,
    )
...
def test_supervised_uno_acceptance_rejects_missing_source_content(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    first = tmp_path / "Part 1.docx"
...
def test_supervised_uno_acceptance_rejects_source_revision_change(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    original_expected = libreoffice_uno_acceptance.expected_libreoffice_uno_content

    def mutating_expected(
        sources: tuple[Path, ...],
    ) -> libreoffice_uno_acceptance.LibreOfficeUnoContentSnapshot:
    ...
    monkeypatch.setattr(
        libreoffice_uno_acceptance,
        "expected_libreoffice_uno_content",
        mutating_expected,
    )

    with pytest.raises(ValidationError, match="Source integrity violation"):
        libreoffice_uno_acceptance.run_libreoffice_uno_acceptance(
            [source],
            output,
        )
...
def test_supervised_uno_acceptance_requires_python_uno_bridge(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = tmp_path / "Part 1.docx"
...
def test_supervised_uno_acceptance_rejects_duplicate_sources(tmp_path: Path) -> None:
    source = tmp_path / "Part 1.docx"
    _write_source(source, "Part 1", "Alpha")

    with pytest.raises(ValidationError, match="Duplicate LibreOffice UNO source"):
        libreoffice_uno_acceptance.run_libreoffice_uno_acceptance(
            [source, source],
            tmp_path / "merged.docx",
        )
